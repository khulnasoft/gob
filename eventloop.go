...
// eventloop listens to worker errors (from execution path), worker events (from a pulsebus subscription), and
// signal interrupts. Is responsible for handling each event relative to a given UI to coordinate eventing until
// an eventual graceful exit.
//
//nolint:gocognit,funlen
func eventloop(ctx context.Context, log logger.Logger, subscription *pulsebus.Subscription, workerErrs <-chan error, ux UI) error {
	var events <-chan pulsebus.Event
	if subscription != nil {
		events = subscription.Events()
	} else {
		noEvents := make(chan pulsebus.Event)
		close(noEvents)
		events = noEvents
	}
	...
	for {
		...
		case e, isOpen := <-events:
			...
			if err := ux.Handle(e); err != nil {
				if errors.Is(err, pulsebus.ErrUnsubscribe) {
					events = nil
				} else {
					retErr = append(retErr, err)
