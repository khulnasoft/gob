...
func ConfigCommand(app Application, opts *ConfigCommandConfig) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			var err error
			if opts.LoadConfig {
				err = loadAllConfigs(cmd, internalApp.setupConfig.GifConfig, allConfigs)
			}
			summary := summarizeConfig(cmd, internalApp.setupConfig.GifConfig, opts.makeFilters(internalApp.state.RedactStore), allConfigs)
...
func loadAllConfigs(cmd *cobra.Command, gfiCfg gfi.Config, allConfigs []any) error {
	var errs []error
	for _, cfg := range allConfigs {
		...
		if err := gfi.Load(gfiCfg, cmd, cfg); err != nil {
...
func summarizeConfig(commandWithRootParent *cobra.Command, gfiCfg gfi.Config, redact func(string) string, allConfigs []any) string {
	summary := gfi.SummarizeCommand(gfiCfg, commandWithRootParent, redact, allConfigs...)
...
func summarizeLocationsCommand(internalApp *application) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			summary := summarizeLocations(internalApp.setupConfig.GifConfig, suffix)
...
func summarizeLocations(gfiCfg gfi.Config, onlySuffix string) string {
	out := ""
	for _, f := range gfi.SummarizeLocations(gfiCfg) {
