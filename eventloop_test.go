...
const exitEvent pulsebus.EventType = "testing-exit"

var _ UI = (*uiMock)(nil)

type uiMock struct {
	t            *testing.T
	finalEvent   pulsebus.Event
	subscription pulsebus.Unsubscribable
	mock.Mock
}

func (u *uiMock) Setup(subscription pulsebus.Unsubscribable) error {
	u.t.Logf("UI Setup called")
	u.subscription = subscription
	return u.Called(subscription.Unsubscribe).Error(0)
}

func (u *uiMock) Handle(event pulsebus.Event) error {
...
func Test_EventLoop_unsubscribeError(t *testing.T) {
	test := func(t *testing.T) {
		...
		ux.On("Handle", finalEvent).Return(pulsebus.ErrUnsubscribe)
		// ensure the mock sees basic setup/teardown events
		ux.On("Setup", mock.AnythingOfType("func() error")).Return(nil)
		ux.On("Teardown", false).Return(nil)
