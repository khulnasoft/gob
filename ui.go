...
type UI interface {
	Setup(subscription pulsebus.Unsubscribable) error
	pulsebus.Handler
...
func (u *UICollection) Setup(subscription pulsebus.Unsubscribable) error {
	u.lock.Lock()
	defer u.lock.Unlock()
...
func (u UICollection) Handle(event pulsebus.Event) error {
	u.lock.Lock()
	defer u.lock.Unlock()
	if u.active == nil {
		return nil
